...
class TestOperations(object):
	...
	def test_get_room_occupants(self):
		assert_equal(get_room_occupants('DAUGHTRY'), 2, 'Room has more occupants')
	...
	def test_validate_room_name(self):
		...
		assert_equal(validate_room_name('No Room Name'), 'Invalid Room Name', 'Invalid Room Name')

	def test_room_created(self):
		'''Assert if room was created'''
		roomImp = RoomImplementation()
		assert_equal(roomImp.create_room('DAUGHTRY'), 'Room name Unavailable', 'Room name already used')
		with mock.patch('__builtin__.raw_input', side_effect=['A']):
			creation_status = roomImp.create_room('OCULUS')
		 	assert_equal(creation_status, 'Room Created', 'Room name already used')
	...
	def test_reallocate_person(self):
		...
		room = session.query(Person).filter_by(id_number=25904563).one()
		room_name = room.office
		staff = Staff()
		fellow = Fellow()
		assert_equal(staff.reallocate_person(25904563,room_name), 'Person is in that Room')
		assert_equal(staff.reallocate_person(25904563,'COLDPLAY'), 'Reallocation Successful')
		assert_equal(fellow.reallocate_person(25453567,'LAVENDER'), 'Invalid Reallocation')
		assert_equal(staff.reallocate_person(43566763,'OCULUS'), 'Id Missing')
	

	def test_save_person(self):
		staff = Staff()
		assert_equal(staff.add_person('KOBE BRYANT', 'basketballer', 'Y'), 'Invalid Position' )
		assert_equal(staff.add_person('KOBE', 'fellow', 'Y'), 'Invalid Name')
		
		with mock.patch('__builtin__.raw_input', side_effect=['25900567', 'M']):
			assert_equal(staff.add_person('KEVIN GATES', 'STAFF') , 'Person Saved')

		fellow = Fellow()
		with mock.patch('__builtin__.raw_input', side_effect=['25900000', 'F']):
			assert_equal(fellow.add_person('RUTH BOCHERE', 'fellow', 'Y') , 'Person Saved')


	def test_validate_position(position):
		person = Staff()
		assert_equal(person.validate_position('student'), 'Invalid Position', 'Invalid position')
		assert_equal(person.validate_position('staff'), True, 'Invalid position')
		assert_equal(person.validate_position('fellow'), True, 'Invalid position')

	def test_validate_person_id(self):
		fellow = Fellow()
		assert_equal(fellow.validate_person_id_number(25453567), True, 'Invalid ID')
		assert_equal(fellow.validate_person_id_number(23454388), 'Id Missing', 'Invalid ID')

	def test_print_room(self):
		assert_equal(print_room_details('Invalid Room Name'), 'Invalid Room Name', 'Failed to print room information')

	def test_check_db_name(self):
		amity = Amity()
		assert_equal(amity.check_db_name('tests/test_amity.db'), True, 'Error occurred')
		assert_equal(amity.check_db_name('No file'), 'File Does Not Exist', 'Error occurred')
		import io
		with io.FileIO("tests/db.txt", "w") as file:
			file.write("Hello!")
		assert_equal(amity.check_db_name('tests/db.txt'), 'Not SQLite file', 'Error occurred')


	def test_validate_db_tables(self):
		assert_equal(validate_sqlite_db('tests/test_amity.db'), True, 'Error occurred')
